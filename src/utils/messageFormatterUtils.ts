...
function getMessagesForAnalysis(request: any) {
    ...
    try {
        const systemMessage: SystemChatMessage = new SystemChatMessage("You are the best sales person in the world who can very easily understand information and generate summaries and insights from it.")

        const humanMessage: HumanChatMessage = new HumanChatMessage("I have some meeting notes from a call with a lead : \n" +
            `${request.call_notes} \n` +
...
export { getMessagesInLangchainFormat, getMessagesForOutreach, getMessagesForAnalysis }